...
namespace usb_cam
{
  ...
  public:
    ...
    void onInit() {
      ...
      NODELET_INFO("Starting '%s' (%s) at %dx%d via %s (%s) at %i FPS", camera_name_.c_str(), video_device_name_.c_str(), image_width_, image_height_,
    ...
    bool service_toggle_cap(std_srvs::SetBool::Request& req, std_srvs::SetBool::Response& res) {
      if (req.data) {
        ...
        if (!cam_.is_capturing()) {
          ...
          cam_.start(video_device_name_.c_str(), io_method_, pixel_format_, image_width_, image_height_, framerate_);
          set_v4l_params_();
          cam_lock.unlock();
          try {
            if (publisher_thread_.joinable()) {
    ...
    void spin() {
      if (io_method_ == UsbCam::IO_METHOD_UNKNOWN || pixel_format_ == UsbCam::PIXEL_FORMAT_UNKNOWN) {
        NODELET_ERROR("Unknown IO method %s or Pixel format %s", io_method_name_.c_str(), pixel_format_name_.c_str());
        return;
      }
